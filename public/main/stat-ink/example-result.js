function getExampleResult() {
  return {
    player_rank: 26,
    fes_point: 9,
    type: 'fes',
    other_team_fes_theme: {
      name: 'Mayo',
      key: 'alpha',
      color: {
        g: 0.914,
        css_rgb: 'rgb(100%,91%,61%)',
        b: 0.609,
        r: 1,
        a: 1,
      },
    },
    my_team_members: [
      {
        player: {
          udemae: {
            name: 'B+',
          },
          fes_grade: {
            name: 'Ketchup Champion',
            rank: 3,
          },
          clothes_skills: {
            main: {
              image:
                '/images/skill/67e4e8ec069dfaec1d732c7fe407a2c73e8e51b8.png',
              id: '108',
              name: 'Ability Doubler',
            },
            subs: [
              {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
              {
                name: 'Special Charge Up',
                id: '5',
                image:
                  '/images/skill/1378f3963526d7216ec44da35b924b81a8ff6a37.png',
              },
              null,
            ],
          },
          shoes: {
            rarity: 2,
            brand: {
              name: 'Enperry',
              frequent_skill: {
                image:
                  '/images/skill/34e114a50a001778a574f7061039d43e632137b7.png',
                id: '10',
                name: 'Sub Power Up',
              },
              image:
                '/images/brand/de96243d58e41e928d30290162a6f496033da868.png',
              id: '16',
            },
            name: 'Red & Black Squidkid IV',
            kind: 'shoes',
            thumbnail:
              '/images/shoes/da837fbe9d849434a0036855bd13f65d64f620e2.png',
            image: '/images/shoes/9b01ff74921b71ad2159a2fcf058aa20655bd390.png',
            id: '2017',
          },
          head_skills: {
            main: {
              name: 'Comeback',
              id: '103',
              image:
                '/images/skill/bdc5135874439cf3169d9a54b3f1fbdba3731b34.png',
            },
            subs: [
              {
                id: '3',
                image:
                  '/images/skill/7de1bdfd875ef470b6066c17bed726b5b5113d48.png',
                name: 'Run Speed Up',
              },
              null,
              null,
            ],
          },
          principal_id: '62359a7bd8d03ca7',
          head: {
            kind: 'head',
            brand: {
              name: 'Zekko',
              frequent_skill: {
                id: '6',
                image:
                  '/images/skill/d83b962b84fcea9d02c591c296234f5de77f9682.png',
                name: 'Special Saver',
              },
              id: '4',
              image:
                '/images/brand/f3d01187fd633e7d48d9e4e16ef31da73279293c.png',
            },
            name: 'Five-Panel Cap',
            rarity: 1,
            id: '1007',
            thumbnail:
              '/images/head/fc084d60fd83cda87af66ab2325bad3b66d2b29e.png',
            image: '/images/head/de579a9450d11d96a8c7f5b2cbb0a4fde6b528a9.png',
          },
          player_rank: 35,
          nickname: '\u2605tia ><>\u00b0',
          shoes_skills: {
            main: {
              image:
                '/images/skill/1378f3963526d7216ec44da35b924b81a8ff6a37.png',
              id: '5',
              name: 'Special Charge Up',
            },
            subs: [
              {
                name: 'Sub Power Up',
                image:
                  '/images/skill/34e114a50a001778a574f7061039d43e632137b7.png',
                id: '10',
              },
              {
                id: '13',
                image:
                  '/images/skill/efa003501e1152ef7b617b9e01517c915e05b7ac.png',
                name: 'Cold-Blooded',
              },
              {
                id: '10',
                image:
                  '/images/skill/34e114a50a001778a574f7061039d43e632137b7.png',
                name: 'Sub Power Up',
              },
            ],
          },
          clothes: {
            thumbnail:
              '/images/clothes/3690ef3f13e50f138eadcaa7043fd2351825f90a.png',
            image:
              '/images/clothes/01356e2f6dfbaa2d5a36bd2a45a8b6c7ce0df87f.png',
            id: '26000',
            rarity: 2,
            brand: {
              id: '0',
              image:
                '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
              name: 'SquidForce',
              frequent_skill: {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
            },
            name: 'Splatfest Tee',
            kind: 'clothes',
          },
          star_rank: 0,
          weapon: {
            name: 'Rapid Blaster',
            sub: {
              name: 'Ink Mine',
              image_a:
                '/images/sub/2705be54dafc7a426539c9e8f701197b7a9373f5.png',
              image_b:
                '/images/sub/2e2d1dca678ca3b3e011c3e4620c5c0fa31a9248.png',
              id: '5',
            },
            image:
              '/images/weapon/72bdcf5f6077bd7149832153034b3f43d16ac461.png',
            thumbnail:
              '/images/weapon/0a362970b38864f80cca68fed6deb43354333703.png',
            id: '240',
            special: {
              id: '2',
              image_b:
                '/images/special/9e89e1d67803c3021203182ecc7f38bc2c0f5400.png',
              image_a:
                '/images/special/18990f646c551ee77c5b283ec814e371f692a553.png',
              name: 'Splat-Bomb Launcher',
            },
          },
        },
        death_count: 3,
        special_count: 5,
        assist_count: 1,
        sort_score: 0,
        game_paint_point: 940,
        kill_count: 5,
      },
      {
        kill_count: 4,
        game_paint_point: 588,
        sort_score: 0,
        assist_count: 0,
        death_count: 4,
        player: {
          clothes_skills: {
            main: {
              image:
                '/images/skill/67e4e8ec069dfaec1d732c7fe407a2c73e8e51b8.png',
              id: '108',
              name: 'Ability Doubler',
            },
            subs: [
              {
                name: 'Ink Recovery Up',
                image:
                  '/images/skill/c14f4471b26e0f918c736b5c17e03212290f4541.png',
                id: '2',
              },
              {
                id: '1',
                image:
                  '/images/skill/da8ff08954fd5d890fc8bc4dd4cb761e2a33b703.png',
                name: 'Ink Saver (Sub)',
              },
              null,
            ],
          },
          fes_grade: {
            name: 'Ketchup Defender',
            rank: 2,
          },
          shoes: {
            kind: 'shoes',
            name: 'Snow Delta Straps',
            brand: {
              id: '9',
              image:
                '/images/brand/80648c6e427eae4d32677797fa1be7c0e253fda5.png',
              name: 'Inkline',
              frequent_skill: {
                name: 'Bomb Defense Up',
                image:
                  '/images/skill/35f4ec4284fc5a19da58ffb1a7988eb26eb8bd7f.png',
                id: '12',
              },
            },
            rarity: 2,
            id: '4009',
            thumbnail:
              '/images/shoes/764963b030971bd49debb604c873b8be6dd6a734.png',
            image: '/images/shoes/e0ecfa3d54c46c7aa22ca4d23691bbe1674c9c27.png',
          },
          head_skills: {
            main: {
              name: 'Ink Recovery Up',
              image:
                '/images/skill/c14f4471b26e0f918c736b5c17e03212290f4541.png',
              id: '2',
            },
            subs: [
              {
                id: '5',
                image:
                  '/images/skill/1378f3963526d7216ec44da35b924b81a8ff6a37.png',
                name: 'Special Charge Up',
              },
              {
                name: 'Special Charge Up',
                image:
                  '/images/skill/1378f3963526d7216ec44da35b924b81a8ff6a37.png',
                id: '5',
              },
              {
                name: 'Cold-Blooded',
                image:
                  '/images/skill/efa003501e1152ef7b617b9e01517c915e05b7ac.png',
                id: '13',
              },
            ],
          },
          player_rank: 33,
          nickname: 'Radio',
          shoes_skills: {
            main: {
              id: '0',
              image:
                '/images/skill/04b1de71fba1f14197b9163503955c52fd74858b.png',
              name: 'Ink Saver (Main)',
            },
            subs: [
              {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
              {
                image:
                  '/images/skill/d138c293c8ddac42fadf0e6531100a88c79c81f6.png',
                id: '4',
                name: 'Swim Speed Up',
              },
              {
                name: 'Special Power Up',
                id: '7',
                image:
                  '/images/skill/f20a3e85feeb6b4bb021d28059afd6265cee0b43.png',
              },
            ],
          },
          principal_id: '4d4f2f3fc68386c5',
          head: {
            brand: {
              id: '6',
              image:
                '/images/brand/dee59c0797a6214114e527dfa51f0dd012085172.png',
              frequent_skill: {
                name: 'Ink Saver (Sub)',
                image:
                  '/images/skill/da8ff08954fd5d890fc8bc4dd4cb761e2a33b703.png',
                id: '1',
              },
              name: 'Firefin',
            },
            name: 'Blowfish Bell Hat',
            kind: 'head',
            rarity: 0,
            image: '/images/head/48d0dddeb8d3e185bf0b1e11f4db76cfea48b703.png',
            thumbnail:
              '/images/head/17bf509150d7d36438e0419d72aad4efd0a6354b.png',
            id: '4003',
          },
          clothes: {
            brand: {
              id: '0',
              image:
                '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
              name: 'SquidForce',
              frequent_skill: {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
            },
            name: 'Splatfest Tee',
            kind: 'clothes',
            rarity: 2,
            thumbnail:
              '/images/clothes/3690ef3f13e50f138eadcaa7043fd2351825f90a.png',
            image:
              '/images/clothes/01356e2f6dfbaa2d5a36bd2a45a8b6c7ce0df87f.png',
            id: '26000',
          },
          star_rank: 0,
          weapon: {
            name: 'Flingza Roller',
            sub: {
              name: 'Splash Wall',
              id: '9',
              image_b:
                '/images/sub/42940c5ac8fea75599f8d8379f9e0c5bbd2eaedd.png',
              image_a:
                '/images/sub/b00df7ddec57c9254094c901dbb7533b61e5a3e3.png',
            },
            id: '1030',
            special: {
              image_a:
                '/images/special/18990f646c551ee77c5b283ec814e371f692a553.png',
              image_b:
                '/images/special/9e89e1d67803c3021203182ecc7f38bc2c0f5400.png',
              id: '2',
              name: 'Splat-Bomb Launcher',
            },
            thumbnail:
              '/images/weapon/d0145bac97eabd1046263c8856f3e14c68547a90.png',
            image:
              '/images/weapon/e32ed68bb18628c5ede5816a2fbc2b8fcdd04124.png',
          },
        },
        special_count: 1,
      },
      {
        special_count: 1,
        death_count: 6,
        player: {
          principal_id: '371c06197fcb9f4b',
          head: {
            rarity: 0,
            kind: 'head',
            name: 'Squid Facemask',
            brand: {
              image:
                '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
              id: '0',
              name: 'SquidForce',
              frequent_skill: {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
            },
            id: '8007',
            thumbnail:
              '/images/head/eb399657e03cac4fd910bb99fc18840e407999a7.png',
            image: '/images/head/233e609f7116f497341505a832d44f14196af96b.png',
          },
          player_rank: 41,
          nickname: 'ThatsSo',
          shoes_skills: {
            subs: [
              {
                id: '9',
                image:
                  '/images/skill/daf6883039afa62da91eb93eb2a40b673f10b715.png',
                name: 'Quick Super Jump',
              },
              null,
              null,
            ],
            main: {
              image:
                '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
              id: '11',
              name: 'Ink Resistance Up',
            },
          },
          fes_grade: {
            name: 'Ketchup Champion',
            rank: 3,
          },
          clothes_skills: {
            subs: [
              {
                name: 'Sub Power Up',
                id: '10',
                image:
                  '/images/skill/34e114a50a001778a574f7061039d43e632137b7.png',
              },
              null,
              null,
            ],
            main: {
              image:
                '/images/skill/67e4e8ec069dfaec1d732c7fe407a2c73e8e51b8.png',
              id: '108',
              name: 'Ability Doubler',
            },
          },
          shoes: {
            id: '3002',
            thumbnail:
              '/images/shoes/df60152f85eb1c8ece76b4ef70cd2fec6b740a58.png',
            image: '/images/shoes/eea4ed40f08bea5fa520fbfe1e41950221194ed2.png',
            kind: 'shoes',
            name: 'Neon Sea Slugs',
            brand: {
              name: 'Tentatek',
              frequent_skill: {
                image:
                  '/images/skill/c14f4471b26e0f918c736b5c17e03212290f4541.png',
                id: '2',
                name: 'Ink Recovery Up',
              },
              image:
                '/images/brand/02286fe17bb6bc3f5f13c6b251ddc0a55c44c756.png',
              id: '10',
            },
            rarity: 0,
          },
          head_skills: {
            subs: [
              {
                name: 'Swim Speed Up',
                id: '4',
                image:
                  '/images/skill/d138c293c8ddac42fadf0e6531100a88c79c81f6.png',
              },
              {
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
                name: 'Ink Resistance Up',
              },
              null,
            ],
            main: {
              name: 'Ink Saver (Main)',
              id: '0',
              image:
                '/images/skill/04b1de71fba1f14197b9163503955c52fd74858b.png',
            },
          },
          weapon: {
            name: 'Octobrush',
            sub: {
              name: 'Autobomb',
              image_b:
                '/images/sub/f0e4b4bad0e37031b8cb8ff397d4bb0ad9c86307.png',
              image_a:
                '/images/sub/457de86fa079df54c7a3c96decca49a55a1686ae.png',
              id: '4',
            },
            special: {
              name: 'Inkjet',
              image_a:
                '/images/special/9871c82952ed0141be0310ace1942c9f5f66d655.png',
              image_b:
                '/images/special/26e8117808ce17dadb0f23943359e5909fef4085.png',
              id: '8',
            },
            id: '1110',
            thumbnail:
              '/images/weapon/5f565ede7ae220d5b1d453b3ef55b8d20d7b9a2a.png',
            image:
              '/images/weapon/f1d5740dfb7d87f7e43974bbe5585445368741b8.png',
          },
          star_rank: 0,
          clothes: {
            rarity: 2,
            kind: 'clothes',
            name: 'Splatfest Tee',
            brand: {
              name: 'SquidForce',
              frequent_skill: {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
              image:
                '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
              id: '0',
            },
            id: '26000',
            image:
              '/images/clothes/01356e2f6dfbaa2d5a36bd2a45a8b6c7ce0df87f.png',
            thumbnail:
              '/images/clothes/3690ef3f13e50f138eadcaa7043fd2351825f90a.png',
          },
        },
        kill_count: 6,
        game_paint_point: 742,
        sort_score: 0,
        assist_count: 0,
      },
    ],
    my_team_result: {
      name: 'DEFEAT',
      key: 'defeat',
    },
    my_estimate_fes_power: 1870,
    rule: {
      name: 'Turf War',
      multiline_name: 'Turf\nWar',
      key: 'turf_war',
    },
    weapon_paint_point: 2620,
    star_rank: 0,
    other_team_members: [
      {
        special_count: 3,
        player: {
          weapon: {
            name: '.52 Gal',
            image:
              '/images/weapon/df04ddaf086cea94491df553a6d2550230a4da3c.png',
            thumbnail:
              '/images/weapon/3e69eaa1b1463689ddbe4fe23661f007dc216b08.png',
            id: '50',
            special: {
              id: '11',
              image_b:
                '/images/special/caf32476e5010e27a4ef9923dbe323978b1d7510.png',
              image_a:
                '/images/special/3e9cd3e60367fbe82d6237a10fdc826d695b9fa0.png',
              name: 'Baller',
            },
            sub: {
              name: 'Point Sensor',
              id: '8',
              image_b:
                '/images/sub/627ae0ea02ab649a7a482ee7ee7ace85ca307f44.png',
              image_a:
                '/images/sub/14b4d4ef62e915e87bd7caa8b99fb4e986caea26.png',
            },
          },
          star_rank: 0,
          clothes: {
            id: '26000',
            image:
              '/images/clothes/01356e2f6dfbaa2d5a36bd2a45a8b6c7ce0df87f.png',
            thumbnail:
              '/images/clothes/3690ef3f13e50f138eadcaa7043fd2351825f90a.png',
            kind: 'clothes',
            brand: {
              frequent_skill: {
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
                name: 'Ink Resistance Up',
              },
              name: 'SquidForce',
              image:
                '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
              id: '0',
            },
            name: 'Splatfest Tee',
            rarity: 2,
          },
          head: {
            rarity: 2,
            kind: 'head',
            brand: {
              image:
                '/images/brand/9ac5752790dd6dbdc7a427df95e1bfe89fe318e0.png',
              id: '2',
              frequent_skill: {
                id: '4',
                image:
                  '/images/skill/d138c293c8ddac42fadf0e6531100a88c79c81f6.png',
                name: 'Swim Speed Up',
              },
              name: 'Krak-On',
            },
            name: 'Hickory Work Cap',
            id: '1020',
            thumbnail:
              '/images/head/db22e3da0309ac33028ef78385a9f1f0047e26ae.png',
            image: '/images/head/13b7b48cd304d3f56c09006b6e6e6511244673f2.png',
          },
          principal_id: 'f47a34eb5b8317c0',
          shoes_skills: {
            main: {
              image:
                '/images/skill/7de1bdfd875ef470b6066c17bed726b5b5113d48.png',
              id: '3',
              name: 'Run Speed Up',
            },
            subs: [
              {
                image:
                  '/images/skill/d138c293c8ddac42fadf0e6531100a88c79c81f6.png',
                id: '4',
                name: 'Swim Speed Up',
              },
              {
                image:
                  '/images/skill/da8ff08954fd5d890fc8bc4dd4cb761e2a33b703.png',
                id: '1',
                name: 'Ink Saver (Sub)',
              },
              {
                name: 'Quick Super Jump',
                id: '9',
                image:
                  '/images/skill/daf6883039afa62da91eb93eb2a40b673f10b715.png',
              },
            ],
          },
          nickname: 'Phazos',
          player_rank: 23,
          head_skills: {
            subs: [
              {
                name: 'Ink Saver (Main)',
                id: '0',
                image:
                  '/images/skill/04b1de71fba1f14197b9163503955c52fd74858b.png',
              },
              {
                name: 'Sub Power Up',
                image:
                  '/images/skill/34e114a50a001778a574f7061039d43e632137b7.png',
                id: '10',
              },
              null,
            ],
            main: {
              name: 'Special Power Up',
              id: '7',
              image:
                '/images/skill/f20a3e85feeb6b4bb021d28059afd6265cee0b43.png',
            },
          },
          clothes_skills: {
            subs: [
              {
                name: 'Run Speed Up',
                image:
                  '/images/skill/7de1bdfd875ef470b6066c17bed726b5b5113d48.png',
                id: '3',
              },
              {
                name: 'Cold-Blooded',
                id: '13',
                image:
                  '/images/skill/efa003501e1152ef7b617b9e01517c915e05b7ac.png',
              },
              null,
            ],
            main: {
              name: 'Ability Doubler',
              image:
                '/images/skill/67e4e8ec069dfaec1d732c7fe407a2c73e8e51b8.png',
              id: '108',
            },
          },
          fes_grade: {
            name: 'Mayo Defender',
            rank: 2,
          },
          shoes: {
            rarity: 2,
            name: 'Gold Hi-Horses',
            brand: {
              name: 'Zink',
              frequent_skill: {
                id: '9',
                image:
                  '/images/skill/daf6883039afa62da91eb93eb2a40b673f10b715.png',
                name: 'Quick Super Jump',
              },
              image:
                '/images/brand/3d4661b3f60f4b74e9bb6760ba7397ecd2502a20.png',
              id: '1',
            },
            kind: 'shoes',
            thumbnail:
              '/images/shoes/142e28d019ec820b3631aea05bfc0dc241739f19.png',
            image: '/images/shoes/e5f3e38f8ba8cc6de19b938369df996291abc955.png',
            id: '2006',
          },
        },
        death_count: 2,
        game_paint_point: 969,
        sort_score: 0,
        assist_count: 1,
        kill_count: 10,
      },
      {
        kill_count: 2,
        sort_score: 0,
        assist_count: 1,
        game_paint_point: 683,
        special_count: 2,
        death_count: 5,
        player: {
          clothes: {
            thumbnail:
              '/images/clothes/3690ef3f13e50f138eadcaa7043fd2351825f90a.png',
            image:
              '/images/clothes/01356e2f6dfbaa2d5a36bd2a45a8b6c7ce0df87f.png',
            id: '26000',
            rarity: 2,
            name: 'Splatfest Tee',
            brand: {
              id: '0',
              image:
                '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
              frequent_skill: {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
              name: 'SquidForce',
            },
            kind: 'clothes',
          },
          weapon: {
            name: 'Octobrush',
            sub: {
              image_b:
                '/images/sub/f0e4b4bad0e37031b8cb8ff397d4bb0ad9c86307.png',
              image_a:
                '/images/sub/457de86fa079df54c7a3c96decca49a55a1686ae.png',
              id: '4',
              name: 'Autobomb',
            },
            special: {
              id: '8',
              image_a:
                '/images/special/9871c82952ed0141be0310ace1942c9f5f66d655.png',
              image_b:
                '/images/special/26e8117808ce17dadb0f23943359e5909fef4085.png',
              name: 'Inkjet',
            },
            id: '1110',
            image:
              '/images/weapon/f1d5740dfb7d87f7e43974bbe5585445368741b8.png',
            thumbnail:
              '/images/weapon/5f565ede7ae220d5b1d453b3ef55b8d20d7b9a2a.png',
          },
          star_rank: 0,
          head_skills: {
            main: {
              id: '9',
              image:
                '/images/skill/daf6883039afa62da91eb93eb2a40b673f10b715.png',
              name: 'Quick Super Jump',
            },
            subs: [
              {
                image:
                  '/images/skill/1378f3963526d7216ec44da35b924b81a8ff6a37.png',
                id: '5',
                name: 'Special Charge Up',
              },
              null,
              null,
            ],
          },
          fes_grade: {
            name: 'Mayo Champion',
            rank: 3,
          },
          clothes_skills: {
            main: {
              image:
                '/images/skill/67e4e8ec069dfaec1d732c7fe407a2c73e8e51b8.png',
              id: '108',
              name: 'Ability Doubler',
            },
            subs: [
              {
                name: 'Ink Resistance Up',
                id: '11',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
              },
              {
                image:
                  '/images/skill/35f4ec4284fc5a19da58ffb1a7988eb26eb8bd7f.png',
                id: '12',
                name: 'Bomb Defense Up',
              },
              {
                id: '3',
                image:
                  '/images/skill/7de1bdfd875ef470b6066c17bed726b5b5113d48.png',
                name: 'Run Speed Up',
              },
            ],
          },
          shoes: {
            rarity: 2,
            kind: 'shoes',
            brand: {
              id: '8',
              image:
                '/images/brand/36bc10db0aa2640c87ad712fc0281515beedcca1.png',
              name: 'Splash Mob',
              frequent_skill: {
                image:
                  '/images/skill/04b1de71fba1f14197b9163503955c52fd74858b.png',
                id: '0',
                name: 'Ink Saver (Main)',
              },
            },
            name: 'Piranha Moccasins',
            id: '2013',
            thumbnail:
              '/images/shoes/d02af08a9514000ea470c27b36f0ea6a129a410c.png',
            image: '/images/shoes/4f59ffc64aca628c03f1652cd6205fbc1f8f82b4.png',
          },
          nickname: 'Zelgan',
          shoes_skills: {
            subs: [
              {
                name: 'Ink Saver (Main)',
                image:
                  '/images/skill/04b1de71fba1f14197b9163503955c52fd74858b.png',
                id: '0',
              },
              {
                name: 'Ink Recovery Up',
                image:
                  '/images/skill/c14f4471b26e0f918c736b5c17e03212290f4541.png',
                id: '2',
              },
              null,
            ],
            main: {
              name: 'Stealth Jump',
              id: '109',
              image:
                '/images/skill/8a3f06a972689b094f762626ff36b3db8ee545b5.png',
            },
          },
          player_rank: 19,
          head: {
            rarity: 2,
            kind: 'head',
            brand: {
              name: 'Takoroka',
              frequent_skill: {
                id: '5',
                image:
                  '/images/skill/1378f3963526d7216ec44da35b924b81a8ff6a37.png',
                name: 'Special Charge Up',
              },
              image:
                '/images/brand/eab05d2d502cf953b4ae034c87e52e8c999339d6.png',
              id: '11',
            },
            name: 'Takoroka Visor',
            id: '6003',
            image: '/images/head/2741e3dd7176795d97c1c98f2ae42b0bca5100f5.png',
            thumbnail:
              '/images/head/641818d13435c824635c5b31c825ad118c408c28.png',
          },
          principal_id: '93e0b422f16368b6',
        },
      },
      {
        kill_count: 2,
        game_paint_point: 863,
        sort_score: 0,
        assist_count: 0,
        special_count: 2,
        death_count: 4,
        player: {
          clothes: {
            kind: 'clothes',
            name: 'Splatfest Tee',
            brand: {
              frequent_skill: {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
              name: 'SquidForce',
              id: '0',
              image:
                '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
            },
            rarity: 2,
            id: '26000',
            image:
              '/images/clothes/01356e2f6dfbaa2d5a36bd2a45a8b6c7ce0df87f.png',
            thumbnail:
              '/images/clothes/3690ef3f13e50f138eadcaa7043fd2351825f90a.png',
          },
          weapon: {
            name: 'Tentatek Splattershot',
            sub: {
              name: 'Splat Bomb',
              image_b:
                '/images/sub/b13bf755b279af83904892fae01cd98c866dfec7.png',
              image_a:
                '/images/sub/d2eaeec524d28ef315a13f8a9e11dd1039cb78aa.png',
              id: '0',
            },
            id: '41',
            special: {
              image_a:
                '/images/special/9871c82952ed0141be0310ace1942c9f5f66d655.png',
              image_b:
                '/images/special/26e8117808ce17dadb0f23943359e5909fef4085.png',
              id: '8',
              name: 'Inkjet',
            },
            thumbnail:
              '/images/weapon/5f519a8b3f436c854dc81ee14bfc3a26aef09ebc.png',
            image:
              '/images/weapon/331d889d8113b794131080c8943e05a3d2c4547d.png',
          },
          star_rank: 0,
          head_skills: {
            subs: [
              {
                name: 'Sub Power Up',
                image:
                  '/images/skill/34e114a50a001778a574f7061039d43e632137b7.png',
                id: '10',
              },
              null,
              null,
            ],
            main: {
              id: '8',
              image:
                '/images/skill/84ab4ba1188849dff63a4314955a53ab103b47df.png',
              name: 'Quick Respawn',
            },
          },
          fes_grade: {
            rank: 2,
            name: 'Mayo Defender',
          },
          clothes_skills: {
            main: {
              name: 'Ability Doubler',
              image:
                '/images/skill/67e4e8ec069dfaec1d732c7fe407a2c73e8e51b8.png',
              id: '108',
            },
            subs: [
              {
                name: 'Special Power Up',
                id: '7',
                image:
                  '/images/skill/f20a3e85feeb6b4bb021d28059afd6265cee0b43.png',
              },
              {
                name: 'Ink Resistance Up',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                id: '11',
              },
              null,
            ],
          },
          shoes: {
            kind: 'shoes',
            brand: {
              id: '8',
              image:
                '/images/brand/36bc10db0aa2640c87ad712fc0281515beedcca1.png',
              name: 'Splash Mob',
              frequent_skill: {
                name: 'Ink Saver (Main)',
                id: '0',
                image:
                  '/images/skill/04b1de71fba1f14197b9163503955c52fd74858b.png',
              },
            },
            name: 'Strapping Reds',
            rarity: 0,
            id: '1009',
            image: '/images/shoes/44da40d2e904af91f3f133cb66a2dcd22aabb5fd.png',
            thumbnail:
              '/images/shoes/f777967b2f8bbaa4a9e16a6cd059bed829afb7c9.png',
          },
          shoes_skills: {
            subs: [
              {
                id: '9',
                image:
                  '/images/skill/daf6883039afa62da91eb93eb2a40b673f10b715.png',
                name: 'Quick Super Jump',
              },
              null,
              null,
            ],
            main: {
              id: '11',
              image:
                '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
              name: 'Ink Resistance Up',
            },
          },
          nickname: 'Bird650',
          player_rank: 11,
          head: {
            id: '1009',
            thumbnail:
              '/images/head/9cefe06f51ee969e7001d093fbe538176ac35aa0.png',
            image: '/images/head/1fcb0ee3d5a3ce88cf81cf85722e30accaa7fbfa.png',
            kind: 'head',
            name: 'Backwards Cap',
            brand: {
              frequent_skill: {
                name: 'Special Saver',
                image:
                  '/images/skill/d83b962b84fcea9d02c591c296234f5de77f9682.png',
                id: '6',
              },
              name: 'Zekko',
              image:
                '/images/brand/f3d01187fd633e7d48d9e4e16ef31da73279293c.png',
              id: '4',
            },
            rarity: 0,
          },
          principal_id: '11ab54c5c9e1c0d9',
        },
      },
      {
        kill_count: 2,
        sort_score: 0,
        game_paint_point: 682,
        assist_count: 3,
        special_count: 1,
        death_count: 5,
        player: {
          clothes: {
            kind: 'clothes',
            brand: {
              name: 'SquidForce',
              frequent_skill: {
                name: 'Ink Resistance Up',
                id: '11',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
              },
              id: '0',
              image:
                '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
            },
            name: 'Splatfest Tee',
            rarity: 2,
            id: '26000',
            thumbnail:
              '/images/clothes/3690ef3f13e50f138eadcaa7043fd2351825f90a.png',
            image:
              '/images/clothes/01356e2f6dfbaa2d5a36bd2a45a8b6c7ce0df87f.png',
          },
          weapon: {
            sub: {
              id: '8',
              image_a:
                '/images/sub/14b4d4ef62e915e87bd7caa8b99fb4e986caea26.png',
              image_b:
                '/images/sub/627ae0ea02ab649a7a482ee7ee7ace85ca307f44.png',
              name: 'Point Sensor',
            },
            image:
              '/images/weapon/aaead5ff0b63cdcb989b211d649b2552bb3e3a1b.png',
            thumbnail:
              '/images/weapon/3abcd1e1152e6d819bee363311edcf4737d14a45.png',
            id: '5030',
            special: {
              id: '0',
              image_a:
                '/images/special/e395a50d51459dcee4db62d8d1fbc4bb263bc326.png',
              image_b:
                '/images/special/4819d9d318668bdd5dab248a23397ec351bc5c60.png',
              name: 'Tenta Missiles',
            },
            name: 'Dualie Squelchers',
          },
          star_rank: 0,
          fes_grade: {
            name: 'Mayo Defender',
            rank: 2,
          },
          shoes: {
            rarity: 2,
            name: 'Pro Trail Boots',
            brand: {
              id: '9',
              image:
                '/images/brand/80648c6e427eae4d32677797fa1be7c0e253fda5.png',
              frequent_skill: {
                id: '12',
                image:
                  '/images/skill/35f4ec4284fc5a19da58ffb1a7988eb26eb8bd7f.png',
                name: 'Bomb Defense Up',
              },
              name: 'Inkline',
            },
            kind: 'shoes',
            image: '/images/shoes/e76c1aebc583ec0655026a00103c930ef50eea13.png',
            thumbnail:
              '/images/shoes/066cb8c9e3c28d06d2e8d0b976bf9325e8ef8fd8.png',
            id: '5002',
          },
          clothes_skills: {
            subs: [
              {
                name: 'Quick Super Jump',
                id: '9',
                image:
                  '/images/skill/daf6883039afa62da91eb93eb2a40b673f10b715.png',
              },
              {
                id: '11',
                image:
                  '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
                name: 'Ink Resistance Up',
              },
              null,
            ],
            main: {
              image:
                '/images/skill/67e4e8ec069dfaec1d732c7fe407a2c73e8e51b8.png',
              id: '108',
              name: 'Ability Doubler',
            },
          },
          head_skills: {
            main: {
              image:
                '/images/skill/03d1b1e0221fc82d0ab26cb8cd3f5f40107b6190.png',
              id: '101',
              name: 'Last-Ditch Effort',
            },
            subs: [
              {
                name: 'Special Saver',
                id: '6',
                image:
                  '/images/skill/d83b962b84fcea9d02c591c296234f5de77f9682.png',
              },
              {
                id: '7',
                image:
                  '/images/skill/f20a3e85feeb6b4bb021d28059afd6265cee0b43.png',
                name: 'Special Power Up',
              },
              {
                image:
                  '/images/skill/daf6883039afa62da91eb93eb2a40b673f10b715.png',
                id: '9',
                name: 'Quick Super Jump',
              },
            ],
          },
          player_rank: 23,
          nickname: 'Fufu\u2122',
          shoes_skills: {
            subs: [
              {
                image:
                  '/images/skill/1378f3963526d7216ec44da35b924b81a8ff6a37.png',
                id: '5',
                name: 'Special Charge Up',
              },
              {
                image:
                  '/images/skill/1378f3963526d7216ec44da35b924b81a8ff6a37.png',
                id: '5',
                name: 'Special Charge Up',
              },
              null,
            ],
            main: {
              name: 'Ink Resistance Up',
              image:
                '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
              id: '11',
            },
          },
          principal_id: 'f0abbbf28fabf243',
          head: {
            kind: 'head',
            name: '18K Aviators',
            brand: {
              frequent_skill: {
                image:
                  '/images/skill/7de1bdfd875ef470b6066c17bed726b5b5113d48.png',
                id: '3',
                name: 'Run Speed Up',
              },
              name: 'Rockenberg',
              id: '3',
              image:
                '/images/brand/451a2d0b5ceb7ea4ec4e47c3ff05eee362e9b722.png',
            },
            rarity: 2,
            id: '3008',
            thumbnail:
              '/images/head/1b49b1fd3c4962b211c267600af93c14dfef18fc.png',
            image: '/images/head/42433b201e1a3b5dab21d6161b436b19a7e07659.png',
          },
        },
      },
    ],
    fes_power: 1799.5,
    my_team_percentage: 43.8,
    battle_number: '454',
    other_team_percentage: 44.3,
    stage: {
      name: 'Inkblot Art Academy',
      image: '/images/stage/5c030a505ee57c889d3e5268a4b10c1f1f37880a.png',
      id: '4',
    },
    other_estimate_fes_power: 1810,
    fes_grade: {
      name: 'Ketchup Champion',
      rank: 3,
    },
    start_time: 1501912106,
    my_team_fes_theme: {
      color: {
        a: 1,
        r: 0.984,
        css_rgb: 'rgb(98%,20%,12%)',
        g: 0.195,
        b: 0.117,
      },
      key: 'bravo',
      name: 'Ketchup',
    },
    game_mode: {
      name: 'Splatfest Battle',
      key: 'fes_team',
    },
    fes_id: 2050,
    other_team_result: {
      name: 'VICTORY',
      key: 'victory',
    },
    player_result: {
      player: {
        weapon: {
          image: '/images/weapon/91b6666bcbfccc204d86f21222a8db22a27d08d0.png',
          thumbnail:
            '/images/weapon/f1419d88f30dfacdb94a8122ccdd8c14bbadc7c4.png',
          special: {
            image_a:
              '/images/special/abe458a7d13d855f1b27135ccf3e96f46f1f3d10.png',
            image_b:
              '/images/special/415bf85acb0b0dcc478316332fe86efb1fe4f203.png',
            id: '1',
            name: 'Ink Armor',
          },
          id: '10',
          sub: {
            id: '0',
            image_b: '/images/sub/b13bf755b279af83904892fae01cd98c866dfec7.png',
            image_a: '/images/sub/d2eaeec524d28ef315a13f8a9e11dd1039cb78aa.png',
            name: 'Splat Bomb',
          },
          name: 'Splattershot Jr.',
        },
        star_rank: 0,
        clothes: {
          thumbnail:
            '/images/clothes/3690ef3f13e50f138eadcaa7043fd2351825f90a.png',
          image: '/images/clothes/01356e2f6dfbaa2d5a36bd2a45a8b6c7ce0df87f.png',
          id: '26000',
          rarity: 2,
          brand: {
            id: '0',
            image: '/images/brand/5547e529d160b188d104e3b68ff4b7566eab9771.png',
            frequent_skill: {
              image:
                '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
              id: '11',
              name: 'Ink Resistance Up',
            },
            name: 'SquidForce',
          },
          name: 'Splatfest Tee',
          kind: 'clothes',
        },
        head: {
          id: '2004',
          image: '/images/head/64b932a8c885c8f32d60081c986c8c0fb5c4d632.png',
          thumbnail:
            '/images/head/2ffeb26c36f24851f2f16917e90178ccb73503c4.png',
          rarity: 2,
          kind: 'head',
          name: 'Special Forces Beret',
          brand: {
            frequent_skill: {
              image:
                '/images/skill/f20a3e85feeb6b4bb021d28059afd6265cee0b43.png',
              id: '7',
              name: 'Special Power Up',
            },
            name: 'Forge',
            id: '5',
            image: '/images/brand/b38b99b3358f587efd1613b72a72c9ca9f81f406.png',
          },
        },
        principal_id: '74a2f8c69f5edca5',
        nickname: 'Selky',
        shoes_skills: {
          main: {
            name: 'Bomb Defense Up',
            image: '/images/skill/35f4ec4284fc5a19da58ffb1a7988eb26eb8bd7f.png',
            id: '12',
          },
          subs: [null, null, null],
        },
        player_rank: 26,
        head_skills: {
          main: {
            id: '100',
            image: '/images/skill/492691ed2a7ce7ec694bf88d3ea1f692b7da1692.png',
            name: 'Opening Gambit',
          },
          subs: [
            {
              name: 'Ink Resistance Up',
              id: '11',
              image:
                '/images/skill/33087a476135074af856151a89a6fe4d1d3a996e.png',
            },
            null,
            null,
          ],
        },
        shoes: {
          kind: 'shoes',
          name: 'Hunting Boots',
          brand: {
            id: '8',
            image: '/images/brand/36bc10db0aa2640c87ad712fc0281515beedcca1.png',
            name: 'Splash Mob',
            frequent_skill: {
              image:
                '/images/skill/04b1de71fba1f14197b9163503955c52fd74858b.png',
              id: '0',
              name: 'Ink Saver (Main)',
            },
          },
          rarity: 2,
          id: '6012',
          image: '/images/shoes/8dc67befb3158733c963c78e0d3205adb2245987.png',
          thumbnail:
            '/images/shoes/01ec0d91527fd71fd81b4c9146972af8fc5212fb.png',
        },
        fes_grade: {
          rank: 3,
          name: 'Ketchup Champion',
        },
        clothes_skills: {
          main: {
            name: 'Ability Doubler',
            image: '/images/skill/67e4e8ec069dfaec1d732c7fe407a2c73e8e51b8.png',
            id: '108',
          },
          subs: [
            {
              id: '9',
              image:
                '/images/skill/daf6883039afa62da91eb93eb2a40b673f10b715.png',
              name: 'Quick Super Jump',
            },
            null,
            null,
          ],
        },
        udemae: {
          name: 'B-',
        },
      },
      death_count: 3,
      special_count: 2,
      game_paint_point: 772,
      sort_score: 0,
      assist_count: 3,
      kill_count: 1,
    },
    max_fes_poser: 2004.2,
    win_meter: 1,
  };
}

module.exports = getExampleResult;
